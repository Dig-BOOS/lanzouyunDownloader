...
🔍 工作原理
读取链接 – 从文本文件加载所有待下载链接，并过滤掉已成功处理的记录。

并发控制 – 使用 asyncio.Semaphore 限制同时运行的页面数量。

模拟浏览器 – 每个链接启动一个 Playwright 页面，访问蓝奏云分享页。

处理密码（如有） – 检测页面中是否有密码输入框，自动填写并提交。

定位 iframe – 蓝奏云下载按钮位于一个 iframe.ifr2 内，脚本会等待 iframe 加载并切换到内部。

点击下载 – 在 iframe 中寻找下载链接（支持多种文字和属性），触发下载。

保存文件 – 监听浏览器下载事件，将文件保存到指定目录；若文件名冲突，自动添加序号。

记录状态 – 成功则写入 processed.txt，最终失败则写入 failed.txt。
...
2026-02-23 12:30:48 - INFO - ✅ 下载成功: 260130专属群《马年年货》.pdf
2026-02-23 12:30:51 - WARNING - ❌ 尝试 1/3 失败: https://lazymans.lanzoue.com/xxx - Timeout
2026-02-23 12:30:56 - INFO - ✅ 下载成功: 260130洪灝付费文.pdf
...
